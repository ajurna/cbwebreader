...
RUN apk add --no-cache --virtual .build-deps gcc build-base python3-dev g++ cmake make mupdf-dev freetype-dev musl-dev libffi-dev postgresql-dev mariadb-dev mariadb-connector-c-dev \
    && apk add --no-cache  tini bash unrar dcron python3 py3-pip mariadb-connector-c py3-wheel jpeg-dev postgresql-libs mupdf freetype \
